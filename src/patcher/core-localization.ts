...
interface LanguagePackInfo {
    ...
    extensionTranslations: LanguagePackTranslation[];
}

interface LanguagePackTranslation {
    id: string;
    path: string;
}
...
interface NlsPatchResult {
    replaced: number;
    changed: boolean;
    changedFile?: string;
}

interface ExtensionPatchResult {
    replaced: number;
    changedFiles: string[];
    patchedFileCount: number;
}

const manualFallbackByKey: Record<string, string> = {
    'view.workbench.scm.empty':
        'Aby korzystać z funkcji usługi Git, możesz otworzyć folder zawierający repozytorium Git albo sklonować je z adresu URL.\n' +
...
const manualFallbackByEnglish: Record<string, string> = {
    'In order to use Git features, you can open a folder containing a Git repository or clone from a URL.\n[Open Folder](command:vscode.openFolder)\n[Clone Repository](command:git.cloneRecursive)\nTo learn more about how to use Git and source control in the IDE [read our docs](https://aka.ms/vscode-scm).':
        'Aby korzystać z funkcji usługi Git, możesz otworzyć folder zawierający repozytorium Git albo sklonować je z adresu URL.\n' +
        '[Otwórz folder](command:vscode.openFolder)\n' +
        '[Sklonuj repozytorium](command:git.cloneRecursive)\n' +
        'Aby dowiedzieć się więcej o korzystaniu z Git i kontroli źródła w IDE, [przeczytaj dokumentację](https://aka.ms/vscode-scm).',
...
export function applyCoreLocalizationFromLanguagePack(paths: CoreLocalizationPaths): CoreLocalizationResult {
    ...
    let nlsResult: NlsPatchResult = { replaced: 0, changed: false };
    try {
    ...
    let extensionResult: ExtensionPatchResult = { replaced: 0, changedFiles: [], patchedFileCount: 0 };
    try {
...
export function restoreCoreLocalizationBackups(paths: CoreLocalizationPaths): { restoredFiles: number; details: string[] } {
    let restoredFiles = 0;
    const details: string[] = [];
...
export function getCorePatchStatus(paths: CoreLocalizationPaths): { polishFound: number; englishFound: number; details: string } {
...
function findPolishLanguagePack(): LanguagePackInfo | null {
...
function getLanguagePackSearchRoots(): string[] {
...
function readLanguagePack(root: string): LanguagePackInfo | null {
    ...
    const polish = localizations.find((entry: any) => entry?.languageId === 'pl');
    if (!polish || !Array.isArray(polish.translations)) {
        return null;
    }

    const translations: LanguagePackTranslation[] = [];
...
function patchCoreMessages(paths: CoreLocalizationPaths, mainTranslationPath: string): NlsPatchResult {
...
function patchBuiltInExtensions(paths: CoreLocalizationPaths, extensionTranslations: LanguagePackTranslation[]): ExtensionPatchResult {
...
function resolvePackageNlsPath(extensionsRoot: string, translationId: string): string | null {
...
function patchPackageNlsObject(sourceJson: Record<string, unknown>, bundle: Record<string, unknown>): { value: Record<string, unknown>; changed: number } {
...
function pickTranslation(bundle: Record<string, unknown>, key: string, english: string): string {
